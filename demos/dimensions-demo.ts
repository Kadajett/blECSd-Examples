...
import { addEntity, AUTO_DIMENSION, createWorld, decodePercentage, encodePercentage, getDimensions, isPercentage, setDimensions, writeRaw, setOutputStream, enterAlternateScreen, hideCursor, showCursor, leaveAlternateScreen } from 'blecsd';
...
function drawBox(x: number, y: number, w: number, color: string): void {
	...
	writeRaw(`\x1b[${y};${x}H${color}${'#'.repeat(dw)}\x1b[0m`);
	writeRaw(`\x1b[${y + 1};${x}H${color}#${' '.repeat(Math.max(0, dw - 2))}#\x1b[0m`);
	writeRaw(`\x1b[${y + 2};${x}H${color}${'#'.repeat(dw)}\x1b[0m`);
}

function render(): void {
	writeRaw('\x1b[H\x1b[2J');
	writeRaw('\x1b[1;3H\x1b[1;36mDimensions Demo\x1b[0m');
	writeRaw(`\x1b[2;3H\x1b[90mTerminal: ${termW}x${termH} | Auto, percentages, absolute sizes\x1b[0m`);
	...
	for (let i = 0; i < items.length; i++) {
		...
		writeRaw(`\x1b[${row};3H${marker}${color}\x1b[1m${e.label}\x1b[0m  \x1b[90m${e.desc}\x1b[0m`);
		writeRaw(`\x1b[${row + 1};7H\x1b[90mStored: w=${dims?.width ?? '?'} h=${dims?.height ?? '?'}\x1b[0m`);
		writeRaw(`\x1b[${row + 1};40H\x1b[32mw=${resolve(e.w, termW)} h=${resolve(e.h, termH)}\x1b[0m`);
	...
	writeRaw(`\x1b[${row};3H\x1b[90mencodePercentage(50)=${encodePercentage(50)} | AUTO_DIMENSION=${AUTO_DIMENSION}\x1b[0m`);
	writeRaw(`\x1b[${Math.min(termH - 1, row + 2)};1H\x1b[33m[Up/Down] Select  [q] Quit\x1b[0m`);
}
