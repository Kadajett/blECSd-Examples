...
import { createWorld, addEntity, attachCheckboxBehavior, isChecked, toggleCheckbox, getCheckboxState, onCheckboxChange, writeRaw, setOutputStream, enterAlternateScreen, hideCursor, showCursor, leaveAlternateScreen } from 'blecsd';
...
function render(): void {
	...
	writeRaw('\x1b[1;3H\x1b[1;36mCheckbox Demo\x1b[0m');
	writeRaw('\x1b[2;3H\x1b[90mToggle checkboxes with Space, navigate with arrows\x1b[0m');
	for (let i = 0; i < items.length; i++) {
		...
		writeRaw(`\x1b[${4 + i};5H${marker}${box} \x1b[37m${item.label}\x1b[0m`);
		writeRaw(`\x1b[${4 + i};50H\x1b[90m(${state})\x1b[0m`);
	}
	const checkedCount = items.filter((it) => isChecked(world, it.eid)).length;
	writeRaw(`\x1b[11;5H\x1b[90mChecked: ${checkedCount}/${items.length}\x1b[0m`);
	if (changes.length > 0) {
		writeRaw('\x1b[13;5H\x1b[90mRecent changes:\x1b[0m');
		for (let i = 0; i < changes.length; i++) writeRaw(`\x1b[${14 + i};7H\x1b[90m${changes[i]}\x1b[0m`);
	}
	writeRaw(`\x1b[${Math.min(height - 1, 20)};1H\x1b[33m[Up/Down] Navigate  [Space] Toggle  [q] Quit\x1b[0m`);
}
