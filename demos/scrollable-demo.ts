...
import { addEntity, canScroll, createWorld, getScroll, getScrollPercentage, isAtBottom, isAtTop, scrollBy, scrollToBottom, scrollToTop, setScrollable, writeRaw, setOutputStream, enterAlternateScreen, hideCursor, showCursor, leaveAlternateScreen } from 'blecsd';
...
function render(): void {
	...
	writeRaw('\x1b[1;3H\x1b[1;36mScrollable Content Demo\x1b[0m');
	const pos = getScroll(world, eid);
	const pct = getScrollPercentage(world, eid);
	const top = isAtTop(world, eid), bot = isAtBottom(world, eid);
	writeRaw(`\x1b[2;3H\x1b[90mScroll: y=${pos.y} | ${Math.round(pct.y)}% | ${top ? 'TOP' : bot ? 'BOTTOM' : 'middle'} | scrollable=${canScroll(world, eid)}\x1b[0m`);
	// Viewport borders
	const [vx, vy] = [3, 4];
	writeRaw(`\x1b[${vy};${vx}H\x1b[36m${'='.repeat(width - 8)}\x1b[0m`);
	writeRaw(`\x1b[${vy + viewH + 1};${vx}H\x1b[36m${'='.repeat(width - 8)}\x1b[0m`);
	// Visible lines
	for (let i = 0; i < viewH; i++) {
		const line = (lines[pos.y + i] ?? '').substring(0, width - 12);
		writeRaw(`\x1b[${vy + 1 + i};${vx}H\x1b[37m${line}\x1b[0m`);
	}
	// Scrollbar
	const sbX = width - 4;
	...
	for (let i = 0; i < viewH; i++) {
		...
		writeRaw(`\x1b[${vy + 1 + i};${sbX}H${ch}`);
	}
	writeRaw(`\x1b[${Math.min(height - 1, vy + viewH + 3)};1H\x1b[33m[Up/Down] Scroll  [PgUp/PgDn] Page  [Home/End] Jump  [q] Quit\x1b[0m`);
}
