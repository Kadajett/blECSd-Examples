...
import { writeRaw, setOutputStream, enterAlternateScreen, hideCursor, showCursor, leaveAlternateScreen } from 'blecsd';
...
function render(): void {
	...
	writeRaw('\x1b[1;3H\x1b[1;36mHover Tooltips Demo\x1b[0m');
	writeRaw('\x1b[2;3H\x1b[90mNavigate with arrows, tooltips appear after delay\x1b[0m');
	for (let i = 0; i < buttons.length; i++) {
		const b = buttons[i]; if (!b) continue;
		const hl = i === idx;
		writeRaw(`\x1b[${b.y};${b.x}H${hl ? '>' : ' '}${hl ? '\x1b[1;33m' : '\x1b[37m'}${b.label}\x1b[0m`);
	}
	const now = Date.now();
	...
	if (tip) {
		...
		writeRaw(`\x1b[${ty};${tx}H\x1b[43;30m${border}\x1b[0m`);
		writeRaw(`\x1b[${ty + 1};${tx}H\x1b[43;30m| ${tip.text} |\x1b[0m`);
		writeRaw(`\x1b[${ty + 2};${tx}H\x1b[43;30m${border}\x1b[0m`);
	}
	writeRaw(`\x1b[${Math.min(height - 1, 12)};1H\x1b[33m[Left/Right] Navigate  [q] Quit\x1b[0m`);
}
