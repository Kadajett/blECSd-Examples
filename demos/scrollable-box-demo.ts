...
function render(): void {
	...
	writeRaw('\x1b[1;3H\x1b[1;36mScrollableBox Widget Demo\x1b[0m');
	writeRaw('\x1b[2;3H\x1b[90mA box with built-in scroll support\x1b[0m');
	const scroll = box.getScroll();
	const [x, y] = [3, 4];
	// Borders
	writeRaw(`\x1b[${y};${x}H\x1b[36m${'╭'}${'─'.repeat(boxW - 2)}${'╮'}\x1b[0m`);
	for (let i = 0; i < viewH; i++) {
		const line = (contentLines[scroll.y + i] ?? '').substring(0, boxW - 4).padEnd(boxW - 4, ' ');
		writeRaw(`\x1b[${y + 1 + i};${x}H\x1b[36m│\x1b[0m \x1b[37m${line}\x1b[0m \x1b[36m│\x1b[0m`);
	}
	writeRaw(`\x1b[${y + viewH + 1};${x}H\x1b[36m${'╰'}${'─'.repeat(boxW - 2)}${'╯'}\x1b[0m`);
	// Scrollbar
	const sbX = x + boxW - 1;
	...
	for (let i = 0; i < viewH; i++) {
		...
		writeRaw(`\x1b[${y + 1 + i};${sbX}H${ch}`);
	}
	const iy = y + viewH + 3;
	writeRaw(`\x1b[${iy};3H\x1b[90mScroll: y=${scroll.y} | ${box.isAtTop() ? 'TOP' : box.isAtBottom() ? 'BOTTOM' : 'middle'} | Lines: ${contentLines.length}\x1b[0m`);
	writeRaw(`\x1b[${Math.min(height - 1, iy + 2)};1H\x1b[33m[Up/Down] Scroll  [PgUp/PgDn] Page  [Home/End] Jump  [q] Quit\x1b[0m`);
}
