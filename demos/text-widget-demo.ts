...
import { createWorld, addEntity, writeRaw, setOutputStream, enterAlternateScreen, hideCursor, showCursor, leaveAlternateScreen } from 'blecsd';
...
function render(): void {
	...
	writeRaw('\x1b[1;3H\x1b[1;36mText Widget Demo\x1b[0m');
	writeRaw('\x1b[2;3H\x1b[90mMultiple text widgets with different alignments\x1b[0m');
	for (let i = 0; i < texts.length; i++) {
		...
		const col = i < 3 ? 3 : 50;
		writeRaw(`\x1b[${row - 1};${col}H${marker}${labels[i]}\x1b[0m`);
		for (let j = 0; j < lines.length; j++) writeRaw(`\x1b[${row + j};${col + 3}H\x1b[37m${lines[j]}\x1b[0m`);
	}
	writeRaw(`\x1b[${Math.min(height - 2, 18)};1H\x1b[33m[Tab] Select  [Space] Update content  [q] Quit\x1b[0m`);
	writeRaw(`\x1b[${Math.min(height - 1, 19)};1H\x1b[90mSelected: ${labels[sel]} | Updates: ${counter}\x1b[0m`);
}
