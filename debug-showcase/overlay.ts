...
export function renderOverlay(
	...
	for (const textLine of worldLines) {
		...
		writeRaw(`\x1b[${line + 1};${startX + 2}H`);
		writeRaw('\x1b[38;2;200;200;255m'); // Light blue
		writeRaw(textLine.slice(0, overlayWidth - 4));
		writeRaw('\x1b[0m');
		line++;
	}
	line++;
	...
	if (line < height - 1) {
		...
		writeRaw(`FPS: ${perfStats.fps.toFixed(1)}`);
		writeRaw('\x1b[0m');
		line++;

		writeRaw(`\x1b[${line + 1};${startX + 2}H`);
		writeRaw('\x1b[38;2;200;255;200m');
		writeRaw(`Frame: ${perfStats.frameTime.toFixed(2)}ms`);
		writeRaw('\x1b[0m');
		line++;

		writeRaw(`\x1b[${line + 1};${startX + 2}H`);
		writeRaw('\x1b[38;2;200;255;200m');
		writeRaw(`Entities: ${perfStats.entityCount}`);
		writeRaw('\x1b[0m');
		line++;
	}
	line++;
