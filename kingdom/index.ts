...
import { screenSeq, cursorSeq, style } from 'blecsd/terminal';
import {
  type GameState,
...
function main(): void {
  ...
  writeRaw(screenSeq.alternateOn());
  writeRaw(cursorSeq.hide());
  ...
  const cleanup = (): void => {
    saveGame(state);
    cleanupInput();
    writeRaw(cursorSeq.show());
    writeRaw(screenSeq.alternateOff());
